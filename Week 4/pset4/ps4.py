...
def generate_models(x, y, degs):
    ...
    for deg in degs:
        ...
        degree_coefficients.append(model)
...
def r_squared(y, estimated):
    ...
    for index in range(len(y)):
        # sum of squares
        error += ((y[index] - estimated[index])**2)
    ...
    observed_variance = sum_of_variance/len(y)   
...
def evaluate_models_on_training(x, y, models):
    ...
    # model plot
    # get the y-values for the model:
    
    model_y_vals = []
    for model in models:
        
        coefficients = (model[0], model[1])
        
        model_y_vals.append(np.polyval(coefficients, x_vals))
        
        Rsq_error = r_squared(y_vals, model_y_vals[0])
        print("r_squared_error for current model", Rsq_error)
...
print("generating model for jan 10 over specified interval")
...
y = []
for year in INTERVAL_1:
    # this evaluates to a list of arrays
    y.append(np.mean(raw_data.get_yearly_temp('BOSTON', year)))
    
print("generating model for yearly average over specified interval")
